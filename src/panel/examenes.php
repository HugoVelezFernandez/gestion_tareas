<?php 
    ...
    $csmdon = 0;
    $sql_find_tareas = "SELECT id FROM examenes WHERE vinc_usuario = $id_token_decode  AND nota = '0'";
...
<body class="body-panel">
    <div class="div-bs-c">
        <div class="barra-superior">
            ...
            <div class="seccion"> <h1 class="titulo-seccion">Exams</h1> </div>
            <div class="apartados">
        ...
        <div class="contenido">
            <div class="div-contenido">
                <div class="div-principal border-g">
                    <h2 class="titulo-md">Exam list</h2>
                    <div class="caja-mostrar-tareas">
                        ...
                        <?php
                            ...
                            if(!empty($_GET['sdh']) AND $_GET['sdh'] == "on"){header('location: examenes_alternativo.php');}

                            // Recojer datos para el paginador
                            $sql_ch2 = "SELECT id FROM examenes WHERE vinc_usuario = $id_token_decode AND realizado = 'No'";
                            if($result = $conn->query($sql_ch2)){while($row = $result->fetch_assoc()){$contar = $contar + 1;}}

                            // Recojer datos y mostrarlos (Datos de examenes)
                            $sql_find_tareas = "SELECT id, asignatura, texto, dia, mes FROM examenes WHERE vinc_usuario = $id_token_decode AND realizado = 'No' LIMIT $inicio_mostrar_sql,$max_mostrar_sql";
                            if($result = $conn->query($sql_find_tareas)){
                                while($row = $result->fetch_assoc()){
                                    $id_homework = $row['id'];
                                    echo '
                                    <div style="display: flex;">
                                        <div class="caja-datos-tareas">'.$row["texto"].': '.$row["asignatura"].'</div>
                                        <div class="caja-datos-tareas ns">'.$row["dia"]." ".$row["mes"].'</div>
                                        <div class="caja-datos-tareas"><a href="actions/cde.php?id='.$id_homework.'"><button class="boton-tareas">Change data</button></a></div>
                                    </div>';
                                }
                            }
                    ...
                    <div class="pages">
                        <input type="checkbox" name="sdh"> Show Realized Exams
                ...
                <div class="div-secundario">
                    <div class="div-sqlinsert border-g">
                        <h2 class="titulo-general-login">Config new exam</h1>
                        <form action="actions/actexam.php" method="post">
                            <div>
                            ...
                            <div>
                                <h4 class="titulo-insert-data">Insert date</h4>
                                <div class="ife">
