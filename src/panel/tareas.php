<?php 
    ...
    // Comprobar si existe "page" para correcto funcionamiento de pagina
    ...
    $csmdon = 0;
    $sql_find_tareas = "SELECT id FROM tareas WHERE vinc_usuario = $id_token_decode  AND entregado = 'No'";
...
<body class="body-panel">
    <div class="div-bs-c">
        <div class="barra-superior">
            ...
            <div class="seccion"> <h1 class="titulo-seccion">Homeworks</h1> </div>
            <div class="apartados">
        ...
        <div class="contenido">
            <div class="div-contenido">
                <div class="div-principal border-g">
                    <h2 class="titulo-md">Homeworks list</h2>
                    <div class="caja-mostrar-tareas">
                        ...
                        <?php
                            // Saltar a otro archivo (Si quieres ver los ocultos)
                            if(!empty($_GET['sdh']) AND $_GET['sdh'] == "on"){header('location: tareas_alternativo.php');}

                            // Recojer datos para el paginador
                            $sql_ch2 = "SELECT id FROM tareas WHERE vinc_usuario = $id_token_decode AND entregado = 'No'";
                            if($result = $conn->query($sql_ch2)){while($row = $result->fetch_assoc()){$contar = $contar + 1;}}

                            // Recojer datos y mostrarlos (Datos de tareas)
                            $sql_find_tareas = "SELECT id, texto, dia_entrega, mes_entrega, entregado FROM tareas WHERE vinc_usuario = '$id_token_decode' AND entregado = 'No' LIMIT $inicio_mostrar_sql,$max_mostrar_sql";
                            if($result = $conn->query($sql_find_tareas)){
                                while($row = $result->fetch_assoc()){
                                    ...
                                    <div style="display: flex;">
                                        ...
                                        <div class="caja-datos-tareas ns">'.$row["dia_entrega"]." ".$row["mes_entrega"].'</div>
                                        <div class="caja-datos-tareas"><a href="actions/cdh.php?id='.$id_homework.'"><button class="boton-tareas">Change data</button></a></div>
                                    </div>';
                                    if(!empty($_GET['cd']) AND $_GET['cd'] != 0){header('Location: actions/cdh.php?id='.$id_homework.'');}
                                }
                            }
                    ...
                    <div class="pages">
                        <input type="checkbox" name="sdh"> Show Delivered Homeworks
                ...
                <div class="div-secundario">
                    <div class="div-sqlinsert border-g">
                        <h2 class="titulo-general-login">Config new homework</h1>
                        <form action="actions/acthomework.php" method="post">
                            <div>
