<?php 
    ...
    $csmdon = 0;
    $sql_find_tareas = "SELECT id FROM trabajos WHERE vinc_usuario = $id_token_decode  AND entregado = 'No'";
                ...
                <div class="div-principal border-g">
                    ...
                    <div class="caja-mostrar-tareas">
                        ...
                        <?php
                            ...
                            if(!empty($_GET['sdh']) AND $_GET['sdh'] == "on"){header('location: trabajos_alternativo.php');}

                            // Recojer datos para el paginador
                            $sql_ch2 = "SELECT id FROM trabajos WHERE vinc_usuario = $id_token_decode AND entregado = 'No'";
                            if($result = $conn->query($sql_ch2)){while($row = $result->fetch_assoc()){$contar = $contar + 1;}}

                            // Recojer datos y mostrarlos (Datos de trabajos)
                            $csmdon = 0;
                            $sql_find_tareas = "SELECT id, asignatura, texto, dia, mes FROM trabajos WHERE vinc_usuario = $id_token_decode  AND entregado = 'No' LIMIT $inicio_mostrar_sql,$max_mostrar_sql";
                            if($result = $conn->query($sql_find_tareas)){
                    ...
                    <div class="pages">
                        <input type="checkbox" name="sdh"> Show Delivered Works
