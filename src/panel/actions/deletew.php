...
<body>
    <div class="caja-login border-g">
        <h1>Are you sure you want to delete this work? </h1>
...
<?php
    ...
    if(!empty($_POST['hid'])){
        if($_POST['hid'] == "yes"){
            $sql_delete = "DELETE FROM trabajos WHERE `trabajos`.`id` = '$id'";
            if(mysqli_query($conn, $sql_delete)){header('Location: ../trabajos.php');}else{echo "<script>alert('Ha ocurrido un error');</script>";}
        }else{header('Location: cdh.php?id='.$id.'');}
    }
?>