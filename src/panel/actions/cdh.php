<?php
    ...
    $sql_find = "SELECT * FROM tareas WHERE id = '$id'";
    if($result = $conn->query($sql_find)){
        while($row = $result->fetch_assoc()){
            $asig = $row['asignatura'];
            $text = $row['texto'];
            $day = $row['dia_entrega'];
            $month = $row['mes_entrega'];
    ...
    if($entregado == "No"){$be = "Yes";}else{$be = "No";}
    if(($plazo == "No") or ($plazo == "Por determinar")){$bp = "Yes";}else{$bp = "No";}
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    ...
    <div class="caja-login border-g">
        ...
        <div>
            ...
            <div><p><b>Realized:</b> <?php echo $realizado; ?> <a href="realizado.php?id=<?php echo $id; ?>"><button class="boton-tareas-2"><?php echo $re; ?></button></a></p></div>
            <div><p><b>Delivered:</b> <?php echo $entregado; ?> <a href="entregado.php?id=<?php echo $id; ?>"><button class="boton-tareas-2"><?php echo $be; ?></button></a></p></div>
            <div><p><b>On time:</b> <?php echo $plazo; ?> <a href="enplazo.php?id=<?php echo $id; ?>"><button class="boton-tareas-2"><?php echo $bp; ?></button></a></p></div>
        </div> 
    </div>

    <div style="text-align: center;"><a href="delete.php?id=<?php echo $id; ?>"><button Class="boton-enviar-delete">Delete</button></a></div>
    ...
    <?php
        if(!empty($_POST['asig'])){
            ...
            $sql_update = "UPDATE `tareas` SET 
                ...
                `texto` = '$pass_update', 
                `dia_entrega` = '$day_update', 
                `mes_entrega` = '$month_update' 
                WHERE `tareas`.`id` = '$id'";
