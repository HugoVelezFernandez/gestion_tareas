...
<?php
    ...
    if(!empty($_POST['hid'])){
        if($_POST['hid'] == "yes"){
            $sql_delete = "DELETE FROM tareas WHERE `tareas`.`id` = '$id'";
            if(mysqli_query($conn, $sql_delete)){header('Location: ../tareas.php');}else{echo "<script>alert('Ha ocurrido un error');</script>";}
